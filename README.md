...
- **커리큘럼은 저장됩니다** — "내 커리큘럼" 탭에서 예전에 만든 것도 다시 볼 수 있고,
  노드마다 "학습 완료" 표시(진행률 바로 확인 가능)와 "AI에게 더 자세히 설명 요청"이
  가능합니다(설명은 한 번 생성하면 캐싱돼서 다시 눌러도 API를 또 호출하지 않아요)
...
curriculum_store.py       커리큘럼 저장/조회/삭제 + 노드 완료 상태 + AI 설명 캐싱 (SQLite)
...
- 커리큘럼 생성은 PDF 다운로드/임베딩/Gemini 생성까지 거치기 때문에 첫 요청에 시간이
  좀 걸릴 수 있습니다. 대신 한 번 만든 커리큘럼은 자동으로 저장되고, 노드를 다시 열면
  그대로 보여줍니다 — 새로고침해도 사라지지 않아요.
